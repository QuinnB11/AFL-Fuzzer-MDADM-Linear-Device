...
int mdadm_read(uint32_t start_addr, uint32_t read_len, uint8_t *read_buf) {
  ...
  while (current_disk <= ldisk_id) {                                                      //Loops through until disks are same
    ...
    while (current_block <= tester) {                                                //Checks block ID for specific disk
      ...
      if (cache_enabled()) {                                                        //Checks if cache is enabled
        if (cache_lookup(current_disk, current_block, temp_buf) == 0) {             //Looks up block in cache and checks if buf is not in cache
          uint32_t op = (0 | JBOD_SEEK_TO_BLOCK << 12) | (current_block << 4);                            // Seeks to block
          jbod_operation(op, NULL);
          op = (JBOD_READ_BLOCK << 12);                                                                   
          jbod_operation(op, temp_buf);                                      //Reads block
          cache_insert(current_disk, current_block, temp_buf);                      //Adds block to cache
        }
        int start_copy = 0;
        int end_copy = JBOD_BLOCK_SIZE - 1;
        if ((current_disk == sdisk_id) && (current_block == sblock_id)) start_copy = srem_bytes;         //Determines start of memcpy
        if ((current_disk == ldisk_id) && (current_block == lblock_id)) end_copy = lrem_bytes;           //Determines end of memcpy
        memcpy(read_buf+offset, &temp_buf[start_copy], (end_copy-start_copy+1));                         //Memcpy from start-end
        offset += end_copy-start_copy+1;                                                                 //Offset of readbuf
        ++current_block;                                                                               //Adds current block pointer
        continue;                                                                                     // Breaks from while loop
        }
...
int mdadm_write(uint32_t start_addr, uint32_t write_len, const uint8_t *write_buf) {
  ...
  while (current_disk <= ldisk_id) {                                          //Loops through disk
    ...
    while (current_block <= tester) {                                   //Loops through blocks
      ...
      if (cache_enabled()) {
        if (cache_lookup(current_disk, current_block, temp_buf) == 1) {                          //Checks if block is already in cache
          cache_update(current_disk, current_block, temp_buf);                       
        } else {
          cache_insert(current_disk, current_block, temp_buf);                                  //Inserts block into cache
        }
      }
